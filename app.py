...
if st.button("📊 Calculate Traffic Severity Index"):
    ...
    # Pre-calculate 24 hours of trends using current user settings
    chart_data = []
    for hr in range(24):
        # Dynamically determine peak state for each simulated hour window
        hr_is_peak = (6 <= hr <= 10 or 16 <= hr <= 21)
        delay_prediction = run_inference(hr, hr_is_peak, corridor_id, rain_prob, vehicle_volume)
        chart_data.append({"Hour": f"{hr}:00", "Estimated Delay (Hours)": round(delay_prediction, 2)})
        
    trend_df = pd.DataFrame(chart_data).set_index("Hour")
    
    # Display clean Streamlit line chart
    ...
    for hr in range(24):
        ...
        # FIX: Use :02d to turn 0:00 into 00:00 and 1:00 into 01:00 for perfect sorting
        chart_data.append({"Hour": f"{hr:02d}:00", "Estimated Delay (Hours)": round(delay_prediction, 2)})
        
    trend_df = pd.DataFrame(chart_data).set_index("Hour")
    
    # Display the correctly sorted chart
    st.line_chart(trend_df)
    st.info("💡 *Insight:* The dual spikes correspond perfectly to your morning and evening peak traffic rush periods combined with your custom weather settings.")
