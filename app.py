...
if st.button("📊 Calculate Traffic Severity Index"):
    ...
    # --- PRODUCTION PLOTLY INTERACTIVE CHART SECTION ---
    st.write("---")
    ...
    for hr in range(24):
        ...
        chart_data.append({"Hour": hr, "Estimated Delay (Hours)": round(delay_prediction, 2)})
        
    trend_df = pd.DataFrame(chart_data)
    
    # Generate custom chronological time string tick labels
    tick_val_list = list(range(0, 24, 2))
    tick_text_list = [f"{t:02d}:00" for t in tick_val_list]
    
    # Create an interactive Plotly line layout
    fig = px.line(
        trend_df, 
        x="Hour", 
        y="Estimated Delay (Hours)",
        labels={"Hour": "Timeline (24h Clock)", "Estimated Delay (Hours)": "Gridlock Delay (Hrs)"},
        template="plotly_dark"
    )
    
    # Inject exact X-axis values matching chronological integers with explicit string formatting labels
    fig.update_layout(
        xaxis=dict(
            tickmode='array',
            tickvals=tick_val_list,
    ...
    # Display the final configured Plotly widget
